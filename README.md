...
1. Sign in with a persistent account.
...
5. Make a final critical decision under pressure.
...
The public `/demo` route contains a precomputed fixture and is explicitly labeled as such. Live assessments require authentication and use the OpenAI API.
...
Open `http://localhost:3000`. With fixtures enabled, the complete deterministic flow can be tested without spending API credits. Never commit `.env.local`.
...
Coverage includes authentication helpers, state transitions, deterministic scoring, evidence validation, API persistence, and the fixture end-to-end path.
